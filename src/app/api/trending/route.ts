...
export async function GET(request: NextRequest) {
  try {
    ...
    // REMOVED: Don't generate images synchronously - this causes 10s timeout!
    // Images will be generated by separate API calls or cached from analysis
    // If no preview exists, it will be null and frontend can handle gracefully

    // Format the response with existing cached images only
    const formattedTrending = trendingRepos.map((item) => ({
      ...
      imageUrl: imageUrlMap.get(item.repoUrl) || null,
    }));
